...
- **特别注意**：兴趣起源和进阶思考板块，**不要写成简历复述**。

**【兴趣起源板块的具体写法】**
- **讲故事为主**（占60-70%）：用具体的场景、人物、对话、感官细节来讲述故事，营造画面感。让人物活起来，有动作、有表情、有对话。
- **适当吹牛**（占15-20%）：可以展现自己独特的视角、别人没注意到的问题、早期"不凡"的想法等。可以有一点小自恋，但要自然。
- **学术理论点缀**（占10-15%）：用理论来升华/点缀故事，而不是大段论述理论。理论是"引用"，不是"主角"。
- **禁止**"我在XXX做YYY"的经历复盘式表达。

**【进阶思考板块的具体写法】**
- **基于兴趣起源的核心问题进行更高阶的思考**
- **增加学术理论深度**：引入相关领域的学者观点、理论框架、研究前沿，让AO感受到你有学术素养
- **展现思辨层次**：不只是"我想更深地了解"，而是"我对这个领域有什么根本性问题有自己的见解"
- 与兴趣起源形成明显的层次递进：从"好奇"到"有见解"，从"困惑"到"能提出问题"
- **禁止**罗列更多经历或重复兴趣起源的内容
...
### 关联线（如有需要）
根据正文内容自主判断是否需要添加跨板块的逻辑关联线，放在顶层 relationships 字段。
如果某些节点之间存在有意义的逻辑呼应或因果关系，则添加关联线；否则可以留空数组。
每个关联线必须包含：
- id: 唯一标识符（UUID）
...
**重要**：end1Title 和 end2Title 必须使用结构中节点的完整标题文本，不要自己编造！宁缺毋滥，不要为了凑数而生成没有逻辑意义的关联线。

### 五大板块结构（固定不变）
1. **兴趣起源 - 用故事+吹牛+少量学术点缀**的方式讲述，不要写成经历复盘。可以从童年回忆、一次难忘的观察、一个"不自量力"的想法切入，重点是展现你独特的思考角度和故事感
2. **进阶思考** - 在兴趣起源基础上展现**认知的演进和思辨深度**，引入更多学术理论框架，提出更高阶的问题/见解。与兴趣起源形成明显层次递进：从"好奇"到"有见解"
...
### JSON 格式（严格遵守，relationships 必须在最外层）
{
  ...
  "structure": [
    {"title": "兴趣起源", "type": "content", "id": "板块1的UUID", "children": [
      ...
      {"title": "这里用3-5条把兴趣起源收口一下哈~", "type": "explanation", "id": "总结节点UUID", "children": []}
    ]},
